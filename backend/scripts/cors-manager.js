...
class CORSManager {
    ...
    try {
      ...
      if (allowedOriginsMatch) {
        const origins = allowedOriginsMatch[1]
          ...
          .map(line => line.trim())
          .filter(
            line => line.startsWith("'http") || line.startsWith('process.env')
          )
          .map(line => {
  ...
  updateAppJs(ports) {
    try {
      ...
      const localhostOrigins = ports
        .map(port => `        'http://localhost:${port}',`)
        .join('\n');
      const ipOrigins = ports
...
${ipOrigins}
  ...
  addPort(port) {
    ...
    if (currentPorts.includes(portNumber)) {
      console.log(
        `ℹ️  Port ${portNumber} is already configured in CORS origins`
      );
      return true;
    }
  ...
  removePort(port) {
    ...
    if (!currentPorts.includes(portNumber)) {
      console.log(
        `ℹ️  Port ${portNumber} is not in the current CORS configuration`
      );
      return true;
    }
  ...
  setPrimaryPort(port) {
    ...
    if (!currentPorts.includes(portNumber)) {
      console.log(
        `⚠️  Port ${portNumber} is not in CORS origins. Adding it first...`
      );
  ...
  showConfig() {
    ...
    console.log(
      `🔗 Allowed Ports: ${currentPorts.length > 0 ? currentPorts.join(', ') : 'None'}`
    );
    console.log('\n📍 Full Origins List:');
    ...
    origins.forEach(origin => {
      if (origin.startsWith('process.env')) {
        console.log(
          `   - ${origin} (${primaryPort ? `http://localhost:${primaryPort}` : 'Not set'})`
        );
      } else {
        console.log(`   - ${origin}`);
      }
